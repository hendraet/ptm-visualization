...
class MascotPreprocessor:
    ...
    def __init__(self, config, preprocessor_config) -> None:
        ...
        self.fasta_headers = preprocessor_helper.process_fasta_files(
    ...
    def reformmods(
        self, sites, peptide, variable_mods, isoform, sequence, aligned_sequence
    ):
        ...
        for i, site in enumerate(sites):
            ...
            if site != 0:
                ...
                if aligned_sequence[offset - 1] != aa:
                    raise ValueError(
                        f"AA don't match for {aa} for peptide {peptide} in sequence {sequence} with offset {offset}"
                    )
            ...
            while line := f.readline():
                if line.startswith('"Variable modifications'):
                    ...
                    if line.startswith("prot_hit_num"):
                        ...
                        if isoform is None:
                            continue
                        reform_mod_strings = self.reformmods(
                            row[pep_var_mod_pos_idx],
                            row[pep_seq_idx],
                            variable_mods,
                            isoform,
                            sequence,
