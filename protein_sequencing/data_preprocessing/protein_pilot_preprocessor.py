...
class ProteinPilotPreprocessor:
    ...
    def get_accession(
        self, row, accession_index, seq_index
    ) -> Tuple[str, str, int, str] | Tuple[None, None, None, None]:
    ...
    def extract_mods_from_rows(
        self, rows, protein_mod_index, mod_index, seq_index, accession_index
    ) -> list:
        """Extract modification strings from rows"""
        mods = []
        for row in rows:
            isoform, sequence, peptide_offset, aligned_sequence = self.get_accession(
                row, accession_index, seq_index
            )
            if (
                isoform is None
                or sequence is None
                or aligned_sequence is None
                or peptide_offset is None
            ):
                continue
            if peptide_offset is None:
                print(
                    f"Error: sequence {row[seq_index]} with assumed accession: {row[accession_index]} not found in fasta file."
                )
                continue
            ...
            for rel_mod in relevant_mods:
                ...
                if matched_mod is not None:
                    ...
                    if self.CONFIG.INCLUDED_MODIFICATIONS.get(mod_name):
                        if (
                            amino_acid
                            not in self.CONFIG.INCLUDED_MODIFICATIONS[mod_name]
                        ):
                            continue
                    ...
                    offset = preprocessor_helper.calculate_exon_offset(
                        mod_pos + peptide_offset + missing_aa,
                        isoform,
                        self.exon_found,
                        self.exon_end_index,
                        self.exon_1_isoforms,
                    ...
                    aligned_offset = (
                        offset
                        - 1
                        + preprocessor_helper.count_missing_aa_in_exon(
                            aligned_sequence,
                            self.exon_start_index,
                            self.exon_end_index,
                            offset,
                    ...
                    if aligned_sequence[aligned_offset] != amino_acid:
                        raise ValueError(
                            f"AA don't match for {amino_acid} for peptide {peptide} in sequence {aligned_sequence} with offset {aligned_offset}"
                        )
    ...
    def extract_cleavages_from_rows(
        self, rows, cleavage_index, seq_index, accession_index
    ) -> list:
        ...
        for row in rows:
            ...
            for site in cleaved_sites:
                ...
                offset = preprocessor_helper.calculate_exon_offset(
                    peptide_offset + site_index + missing_aa,
                    isoform,
                    self.exon_found,
                    self.exon_end_index,
                    self.exon_1_isoforms,
        ...
        for row in rows:
            if "ProteinModifications" in row and "Conf" in row:
                ...
                if row[conf_index] > threshold * 100:
                    if (
                        row[protein_mod_index] is not None
                        and row[protein_mod_index] != ""
                    ):
                        relevant_mod_rows.append(row)
                    if (
                        row[cleavage_index] is not None
                        and row[cleavage_index] != ""
                        and "cleaved" in row[cleavage_index]
                    ):
                        relavent_cleavage_rows.append(row)

        mods = self.extract_mods_from_rows(
            relevant_mod_rows, protein_mod_index, mod_index, seq_index, accession_index
        )
        cleavages = self.extract_cleavages_from_rows(
            relavent_cleavage_rows, cleavage_index, seq_index, accession_index
        )
    ...
    def process_protein_pilot_xlsx_file(self, file) -> Tuple[list, list]:
        ...
        distinct_peptide_level_data = workbook.get_sheet_by_name(
            "Distinct Peptide Level Data"
        )
        ...
        mods, cleavages = self.extract_data_with_threshold(
            peptide_summary, fdr_threshold
        )
    ...
    def process_protein_pilot_dir(self):
        ...
        xlsx_count = len(
            [file for file in os.listdir(self.input_dir) if file.endswith(".xlsx")]
        )
        file_counter = 0
        for file in os.listdir(self.input_dir):
            if file.endswith(".xlsx"):
                mods_for_file, cleavages_for_file = (
                    self.process_protein_pilot_xlsx_file(self.input_dir + file)
                )
        ...
        for _, row in self.groups_df.iterrows():
            if pd.notna(row["replicate"]):
                mods_per_file[row["file_name"]] = mods_per_file[row["file_name"]].union(
                    mods_per_file[row["replicate"]]
                )
                cleavages_per_file[row["file_name"]] = cleavages_per_file[
                    row["file_name"]
                ].union(cleavages_per_file[row["replicate"]])
        ...
        all_cleavages = sorted(
            set(all_cleavages), key=preprocessor_helper.extract_index
        )
        ...
        with open(
            f"{self.CONFIG.OUTPUT_FOLDER}/result_protein_pilot_mods.csv",
            "w",
            newline="",
            encoding="utf-8",
        ) as f:
            ...
            writer.writerow(
                ["", ""]
                + [preprocessor_helper.extract_mod_location(mod) for mod in all_mods]
            )
            ...
            for file, mods in mods_per_file.items():
                ...
                group = self.groups_df.loc[self.groups_df["file_name"] == file][
                    "group_name"
                ].values[0]
                writer.writerow([file[:-10], group] + row)

        with open(
            f"{self.CONFIG.OUTPUT_FOLDER}/result_protein_pilot_cleavages.csv",
            "w",
            newline="",
            encoding="utf-8",
        ) as f:
            ...
            for file, cleavages in cleavages_per_file.items():
                indexes = [
                    preprocessor_helper.extract_index(cleavage)
