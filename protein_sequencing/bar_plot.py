...
def get_bar_plot_width(group_size_a: int, group_size_b: int) -> int:
    """Get width of bar plot."""
    legend_height = utils.get_label_height() * (max(value.count('<br>') + 1 for value in PLOT_CONFIG.BAR_GROUPS.values()))
...
def add_bar_plot(fig: go.Figure, above: str, modification_sights_all: dict[int, list[tuple[int, str, str, str]]], modification_sights_relevant: dict[int, list[tuple[int, str, str, str]]], df, group_positions: list, bar_plot_width: int, label_plot_height: int) -> go.Figure:
    ...
    positions_visited = 0
    bar_percentages = {group: [] for group in PLOT_CONFIG.BAR_GROUPS.keys()}
    ...
    for aa_position in sorted(modification_sights_all.keys(), reverse=True):
        for modification_sight in modification_sights_all[aa_position]:
            if modification_sight not in modification_sights_relevant[aa_position]:
                for group in PLOT_CONFIG.BAR_GROUPS.keys():
            ...
            if CONFIG.FIGURE_ORIENTATION == 0:
                ...
                # plot bars
                for i, group in enumerate(PLOT_CONFIG.BAR_GROUPS.keys()):
                    ...
                    if PLOT_CONFIG.INVERT_AXIS_GROUP_B and above == 'B':
    ...
    max_lines = max(value.count('<br>')+1 for value in PLOT_CONFIG.BAR_GROUPS.values())
    if CONFIG.FIGURE_ORIENTATION == 0:
        for i, group in enumerate(PLOT_CONFIG.BAR_GROUPS.keys()):
...
def plot_line_with_label_horizontal(fig, x_0, x_1, y_0, y_1, y_2, y_3, y_label, color, label, modification_type):
    ...
    fig.add_annotation(x=x_1, y=y_label,
                        ...
                        showarrow=False,
                        textangle=-90,
                        font={'family':CONFIG.FONT, 'size':CONFIG.SEQUENCE_PLOT_FONT_SIZE, 'color':color})
...
def filter_relevant_modification_sights(helper_file: str):
    ...
    for column in df.columns:
        ...
        if column_modification not in PLOT_CONFIG.MODIFICATIONS_GROUP:
            continue
    ...
    # remove rows where groups is not in PLOT_CONFIG.BAR_GROUPS
    header_rows = df.iloc[:4, :]
    df = df[df['Group'].isin(PLOT_CONFIG.BAR_GROUPS.keys())]
