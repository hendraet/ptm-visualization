...
class Plotter:
    ...
    def get_left_margin(self):
        ...
        return int(
            (self.get_label_length(longest_text) / self.get_width() * 1.05)
            * self.get_width()
        )
    ...
    def different_possibilities_plot(
        self, width: int, height: int, different_possibilities: list[int]
    ):
    ...
    def finalize_plotting(
        self, fig, output_path, save_plot: bool = True, show_plot: bool = True
    ):
    ...
    def get_position_with_offset(self, position, isoform):
        ...
        if isoform == "exon2":
            position += (
                self.EXON_1_OFFSET["index_end"] - self.EXON_1_OFFSET["index_start"] + 1
            )
        elif position > max(
            self.EXON_1_OFFSET["index_end"], self.EXON_2_OFFSET["index_end"]
        ):
            if isoform != "general":
                raise ValueError(
                    f"Position {position} is out of range for isoform {isoform}"
                )
            exon_1_length = (
                self.EXON_1_OFFSET["index_end"] - self.EXON_1_OFFSET["index_start"] + 1
            )
            exon_2_length = (
                self.EXON_2_OFFSET["index_end"] - self.EXON_2_OFFSET["index_start"] + 1
            )
    ...
    def _create_plot(
        ...
        if exon_found:
            ...
            max_sequence_length = (
                max_sequence_length - max_exon_length + exon_1_length + exon_2_length
            )
        ...
        max_sequence_length_with_exons = (
            (max_sequence_length + max((exon_1_length, exon_2_length)))
            if exon_found
        ...
        if self.FIGURE_ORIENTATION == 0:
            ...
            self.PIXELS_PER_AA = int(
                (max_sequence_length_pixels - self.EXONS_GAP * exon_found * 2)
                // max_sequence_length_with_exons
            )
        ...
        while region_index < len(self.regions):
            ...
            if exon_found:
                if region_end == exon_1_region[1]:
                    ...
                    self.EXON_1_OFFSET["pixel_start"] = (
                        region_start_pixel + first_exon_offset
                    )
                    self.EXON_1_OFFSET["pixel_end"] = (
                        region_end_pixel + first_exon_offset
                    )
                    region_boundaries.append(
                        (
                            ...
                            region_start_pixel + first_exon_offset,
                            region_end_pixel + first_exon_offset,
                            self.sequence_region_colors[region_group],
                            region_start,
                            region_end,
                            region_plot_type,
                        )
            ...
            region_boundaries.append(
                (
                    region_name,
                    region_start_pixel + exon_offset,
                    region_end_pixel + exon_offset,
                    self.sequence_region_colors[region_group],
                    region_start,
                    region_end,
                    0,
    ...
    def _create_sequence_plot(
        ...
        if legend_positioning:
            ...
            labels = [self.MODIFICATIONS[mod][0] for mod in present_modifications] + [
                self.MODIFICATION_LEGEND_TITLE
            ]
            ...
            if not groups_missing:
                if legend_positioning == "A":
                    x_legend = (
                        0
                        if self.FIGURE_ORIENTATION == 0
                        else width // 2 + self.sequence_plot_height // 2
                    )
                    y_legend = (
                        height // 2
                    ...
                    y_legend = (
                        height // 2 + self.sequence_plot_height // 2
                        if self.FIGURE_ORIENTATION == 0
                        else height
                    )
            else:
    ...
    def _plot_sequence(
        ...
        for i, (
            ...
            region_end,
            exon_type,
        ) in enumerate(region_boundaries):
            if self.FIGURE_ORIENTATION == 0:
